...
export default class CompanyRepository {
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await Company(options.database).deleteOne(
      { _id: id },
      options,
    );
  }
  ...
  static async findAndCountAll(
    ...
    const count =
      await MongooseRepository.wrapWithSessionIfExists(
        Company(options.database).countDocuments(criteria),
        options,
      );
  ...
  static async filterIdsInTenant(
    ...
    let companys = await Company(options.database)
      .find({
        _id: {
          $in: ids,
        },
    ...
    return companys.map((company) => company._id);
  }
  ...
  static async count(filter, options: IRepositoryOptions) {
    return MongooseRepository.wrapWithSessionIfExists(
      Company(options.database).countDocuments(filter),
      options,
    );
  }
}
